...
class Agent():
    ...
    def save_model(self, model_path, metadata=None):
        ...
        checkpoint = {
            ...
            "config": {
                "lr": self.lr,
                "input_dims": list(self.Q_eval.input_dims),
                "fc1_dims": self.Q_eval.fc1_dims,
                "fc2_dims": self.Q_eval.fc2_dims,
    ...
    def load_model(self, model_path, map_location=None):
        """Load current checkpoints and legacy pickled-module model files."""
        device = T.device(map_location) if map_location else self.Q_eval.device
        checkpoint = T.load(model_path, map_location=device)

        if isinstance(checkpoint, nn.Module):
            # Compatibility with the model files included in the original repo.
            self.Q_eval = checkpoint.to(device)
        elif isinstance(checkpoint, dict) and "q_eval_state_dict" in checkpoint:
            self.Q_eval.load_state_dict(checkpoint["q_eval_state_dict"])
            if "q_target_state_dict" in checkpoint:
                self.Q_target.load_state_dict(checkpoint["q_target_state_dict"])
            else:
                self.Q_target.load_state_dict(checkpoint["q_eval_state_dict"])
            self.Q_eval.to(device)
            self.Q_target.to(device)
        else:
            raise ValueError(f"Unsupported DQN checkpoint format: {model_path}")

        self.Q_eval.device = device
        self.Q_eval.eval()
        return checkpoint

    def model_state_hash(self):
        """Return a stable SHA-256 hash of the learned Q-network tensors."""
        digest = hashlib.sha256()
        for name, tensor in sorted(self.Q_eval.state_dict().items()):
